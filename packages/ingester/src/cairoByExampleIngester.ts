...
export async function cleanupDownloadedFiles() {
  const extractDir = path.join(__dirname, 'cairo-by-example');
  await fs.rm(extractDir, { recursive: true, force: true });
  logger.info(`Deleted downloaded markdown files from ${extractDir}`);
}

export async function downloadAndExtractCairoByExample(): Promise<
  BookPageDto[]
> {
  logger.info('Downloading and extracting Cairo By Example');
