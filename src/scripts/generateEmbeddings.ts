import { cairoBookStoreConfig, starknetDocsStoreConfig } from '../config';
...
async function ingestCairoBookData() {
  ...
  try {
    const vectorStore = await VectorStore.initialize(cairoBookStoreConfig);
...
async function ingestStarknetDocsData() {
  ...
  try {
    const vectorStore = await VectorStore.initialize(starknetDocsStoreConfig);
